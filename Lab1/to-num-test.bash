...
#make clean
#make clean all
#make to-num 2> ${PROG}-warn.log > ${PROG}-out.log
...
do
    ${SDIR}/${PROG} < ${JDIR}/${PROG}-${I}.txt > S-tonu${I}.out
    ${JDIR}/${PROG} < ${JDIR}/${PROG}-${I}.txt > J-tonu${I}.out
...
echo "You have ${POINTS} points out of 75"
if [ ${WARNINGS} -ne 0 ]
then
...
if [ ${CLEANUP} -eq 1 ]
then
    rm -f [JS]-tonu[1-${NUM_TESTS}].out ${PROG}-out.log ${PROG}-warn.log
    rm -f ${PROG} ${PROG}.o
fi
